...
class PdbRemote(bdb.Bdb, cmd.Cmd):
    ...
    def remote_run(self, cmd, globals=None, locals=None):
        ...
        try:
            self.remote_client.execute_line(cmd)
    ...
    def interaction(self, frame, tb_or_exc):
        ...
        if PdbRemote._previous_sigint_handler:
            try:
                signal.signal(signal.SIGINT, PdbRemote._previous_sigint_handler)
            except ValueError:  # ValueError: signal only works in main thread
                pass
            else:
                PdbRemote._previous_sigint_handler = None
    ...
    def do_continue(self, arg):
        ...
        if not self.nosigint:
            try:
                PdbRemote._previous_sigint_handler = \
    ...
    def do_debug(self, arg):
        ...
        globals = self.curframe.f_globals
        locals = self.curframe_locals
        p = PdbRemote(self.completekey, self.stdin, self.stdout)
    ...
    def _run(self, target: _ExecutableTarget):
        ...
        print(target.code)
        self.remote_run(self, target.code)
...
if __doc__ is not None:
    ...
    for _command in _help_order:
        __doc__ += getattr(PdbRemote, 'do_' + _command).__doc__.strip() + '\n\n'
    __doc__ += PdbRemote.help_exec.__doc__
...
def run(statement, globals=None, locals=None):
    ...
    """
    PdbRemote().run(statement, globals, locals)
...
def runeval(expression, globals=None, locals=None):
    ...
    """
    return PdbRemote().runeval(expression, globals, locals)
...
def runcall(*args, **kwds):
    ...
    entered.
    """
    return PdbRemote().runcall(*args, **kwds)
...
def post_mortem(t=None):
    ...
    """
    return _post_mortem(t, PdbRemote())
...
def main():
    ...
    if opts.module:
        file = opts.module
        target = pdb._ModuleTarget(file)
    else:
        if not args:
            parser.error("no module or script to run")
        file = args.pop(0)
        if file.endswith('.pyz'):
            target = pdb._ZipTarget(file)
        else:
            target = pdb._ScriptTarget(file)
    ...
    pdbr = PdbRemote()
    pdbr.rcLines.extend(opts.commands)
    while True:
        try:
            pdbr._run(target)
        except Restart:
            ...
            try:
                pdbr.interaction(None, e)
            except Restart:
                print("Restarting", target, "with arguments:")
                print("\t" + " ".join(sys.argv[1:]))
                continue
        if pdbr._user_requested_quit:
            break
